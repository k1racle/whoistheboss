...
5. Только после успешной проверки отдельным изменением удалите публикацию app-порта
   из `ports`. Production PostgreSQL также не публикуется на host: приложение
   подключается к нему по внутренней `guessboss-network`. Для локальной разработки
   порт БД добавляет `docker-compose.dev.yml`.
...
```sh
npm run smoke:assets -- https://маршрут-построен.рф
```

Команда запрашивает основные публичные страницы, извлекает из их HTML все ссылки на CSS и проверяет, что каждый файл отвечает кодом `200` с MIME-типом `text/css`. Ошибка команды блокирует завершение релиза.
