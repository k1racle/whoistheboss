...
### Интеграция Telegram/MAX

Отдельный Python-сервис ботов использует `/api/integrations/bot/*`. Это machine-to-machine API, оно не использует browser session и не должно вызываться из публичного клиента. Каждый запрос подписан HMAC-SHA256 заголовками `X-Bot-Key-Id`, `X-Bot-Timestamp`, `X-Bot-Content-Sha256`, `X-Bot-Signature`; допустимое расхождение часов — 5 минут. Секреты задаются только через `BOT_API_KEY_ID` и `BOT_API_SECRET` в окружении.

Основные ресурсы интеграции:

- `requests` — создание, поиск, карточка, назначение, статус, напоминания и история заявки;
- `events` — последовательная outbox-лента новых заявок, изменений и публикаций;
- `content/*` и `cities` — только опубликованный контент для меню бота;
- `user-data/*` — обезличивание заявок по запросу пользователя;
- `exports/requests.csv` — защищённая административная выгрузка.

`ShootingRequest` остаётся единственным источником заявок для сайта, админки и обоих ботов. Технические состояния диалогов и роли администраторов ботов находятся на VPS бота и не дают прямого доступа к PostgreSQL сайта.
...
После успешного создания, обновления, публикации, снятия с публикации или удаления этих сущностей `admin-content-handlers.ts` передаёт изменённые canonical URL в `server/utils/index-now.ts`. Черновики не отправляются. При смене slug опубликованной записи отправляются и старый, и новый URL. Механизм не меняет JSON-контракты admin API и остаётся выключенным, пока в окружении не задан `INDEXNOW_KEY`.
...
## Обложка и видео главной

Блок «О проекте» использует `HOME_ABOUT_COVER_IMAGE` как статичную обложку и `HOME_ABOUT_VIDEO_URL` / `HOME_ABOUT_VIDEO_FILE` как видео, которое открывается при наведении или нажатии. Старые `HOME_ABOUT_HOVER_VIDEO_*` читаются публичным endpoint как временный fallback и переносятся в основные video-ключи при следующем сохранении формы главной.

## Глобальный порядок бизнеса и предпринимателей

Список `/admin/businesses` задает порядок бизнеса во всех автоматических блоках сайта. Администратор перемещает строки за drag handle, а клиент сохраняет полный массив идентификаторов через `PUT /api/admin/businesses/order`. Сервер валидирует массив и обновляет `Business.placesSortOrder` для всех компаний одной транзакцией.

Публичные подборки бизнеса сортируются по `placesSortOrder`, затем по дате и `id` для стабильного результата. Главная страница показывает первые три карточки. Параметры `limit` и `offset` endpoint `GET /api/businesses` подготавливают последовательную загрузку следующих карточек; объект `pagination` возвращает `hasMore` и `nextOffset`.

Список `/admin/entrepreneurs` работает аналогично и сохраняет полный массив через `PUT /api/admin/entrepreneurs/order` в `Entrepreneur.sortOrder`. Все автоматические подборки предпринимателей используют этот порядок. Новые записи обоих типов добавляются в конец. Миграция `20260828180000_entrepreneur_sort_order` сохраняет прежний порядок существующих предпринимателей по дате создания.
