...
В интерфейсе редактор вводит настоящий перенос через Enter. В строке TypeScript тот же символ можно представить escape-последовательностью `\n`, но редактору не нужно печатать буквальный текст `/n` или `\n`.
